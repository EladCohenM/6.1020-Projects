...
/** String-based commands provided by the Memory Scramble game. @module */
 ...
 * @returns (a promise for) the state of the board from the perspective of playerId,
 *          in the format described in the PS4 handout
 */
export async function look(board: Board, playerId: string): Promise<string> {
    return board.playerBoardState(playerId);
}

/**
 * Tries to flip over a card on the board, following the rules in the PS4 handout.
 * If another player controls the card, then this operation waits until the flip either
 * becomes possible or fails.
 *
 * @param board a Memory Scramble board
 ...
 * @returns (a promise for) the state of the board after the flip from the perspective of playerId,
 *          in the format described in the PS4 handout
 * @throws an error (in a rejected promise) if the flip operation fails as described in the PS4
 *         handout.
 */
 ...
 * other operations like look() and flip() should not throw an unexpected error or wait for the
 * map() to finish.
 *
 * The board must remain observably pairwise consistent for players: if two cards on the board match
 ...
 *
 * Two interleaving map() operations should not throw an unexpected error, or force each other to
 * wait, or violate pairwise consistency, but the exact way they must interleave is not specified.
 *
 * `f` must be a mathematical function from cards to cards: given some legal card `c`, f(c) should
 * be (a promise for) a legal replacement card which is the same every time f(c) is called for that
 * same `c`.
 *
 ...
 * @returns (a promise for) the state of the board after the replacement from the perspective of
 *          playerId, in the format described in the PS4 handout
 */
 ...
 *                 must be a nonempty string of alphanumeric or underscore characters
 * @returns (a promise for) the updated state of the board from the perspective of playerId,
 *          in the format described in the PS4 handout
 */
